// Copied from notion-source.js — no changes needed (pure HTTP API)
...
function extractTitleFromPage(obj) {
  ...
  if (Array.isArray(obj?.title) && obj.title.length > 0) return obj.title.map((t) => t.plain_text || '').join('').trim();
  return '(untitled)';
}
...
function normalizeItem(r) {
  return {
    id: r.id, kind: r.object, title: extractTitleFromPage(r), url: r.url || '',
    modifiedAt: r.last_edited_time || null, createdAt: r.created_time || null,
    parent: normalizeParent(r.parent), hasChildren: r.object === 'database' ? true : (r.has_children === true)
  };
}
...
async function searchNotion(config, { pageSize = 100 } = {}) {
  ...
  if (!active) return { items: [], workspaces, activeWorkspaceId: '', error: '워크스페이스가 없습니다.' };
  try {
    const payload = await notionFetch(NOTION_SEARCH_API, active.token, {
      method: 'POST', body: { page_size: pageSize, sort: { direction: 'descending', timestamp: 'last_edited_time' } }
    });
    const items = (payload.results || []).filter((r) => !r.in_trash && !r.archived).map(normalizeItem);
    return { items, workspaces, activeWorkspaceId: active.id, activeWorkspaceLabel: active.label, fetchedAt: new Date().toISOString() };
  } catch (error) {
    if (error.name === 'AbortError') return { items: [], workspaces, activeWorkspaceId: active.id, activeWorkspaceLabel: active.label, error: 'Notion API 타임아웃 (15s)' };
...
async function fetchPageMeta(pageId, token) {
  try { return normalizeItem(await notionFetch(`${NOTION_PAGES_API}/${pageId}`, token)); } catch (_) { return null; }
}

async function fetchDatabaseMeta(databaseId, token) {
  try { return normalizeItem(await notionFetch(`${NOTION_DATABASES_API}/${databaseId}`, token)); } catch (_) { return null; }
}
  ...
  do {
    ...
    for (const blk of payload.results || []) {
      if (blk.type === 'child_page' || blk.type === 'child_database') {
        children.push({ id: blk.id, kind: blk.type === 'child_page' ? 'page' : 'database', title: blk.type === 'child_page' ? (blk.child_page?.title || '(untitled)') : (blk.child_database?.title || '(untitled)'), has_children: blk.has_children });
      }
    }
...
async function queryDatabaseEntries(databaseId, token, { pageSize = 50 } = {}) {
  try {
    const payload = await notionFetch(`${NOTION_DATABASES_API}/${databaseId}/query`, token, { method: 'POST', body: { page_size: pageSize } });
    return (payload.results || []).filter((r) => !r.in_trash && !r.archived).map(normalizeItem);
  } catch (_) { return []; }
}
...
async function getNotionChildren(config, { parentId, parentKind } = {}) {
  ...
  try {
    if (parentKind === 'database') return { items: await queryDatabaseEntries(parentId, active.token) };
    ...
    const enriched = await Promise.all(stubs.map(async (s) => {
      ...
      return meta || { id: s.id, kind: s.kind, title: s.title, url: '', modifiedAt: null, createdAt: null, parent: { type: 'page', id: parentId }, hasChildren: s.has_children === true };
    }));
